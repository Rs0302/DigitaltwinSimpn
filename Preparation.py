...
for i in range(4):
    ...
    large_trucks[i].attributes["capacity"] = 20
    large_trucks[i].attributes["load_dur"] = 5
    large_trucks[i].attributes["haul_dur"] = 35
    large_trucks[i].attributes["dump_dur"] = 2
    large_trucks[i].attributes["return_dur"] = 20
    p = env.process(truck_process(large_trucks[i], loader_1, loader_2, dumped_dirt))
for i in range(3):
    small_trucks.append(des.Entity(env, "truck"))
    small_trucks[i].attributes["capacity"] = 15
    small_trucks[i].attributes["load_dur"] = 3
    small_trucks[i].attributes["haul_dur"] = 25
    small_trucks[i].attributes["dump_dur"] = 1.5
    small_trucks[i].attributes["return_dur"] = 13
...
print(graphviz.backend.which('dot'))  # Should return the path to dot.exe